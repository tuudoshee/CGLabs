...
static const float gBaseHeightScale = 260.0f;
static const float gPaintHeightScale = 120.0f;
static const float gPaintRootSize = 1024.0f;
...
float SamplePaintMask(float2 worldXZ)
{
    float2 uv;
    uv.x = worldXZ.x / gPaintRootSize + 0.5f;
    uv.y = worldXZ.y / gPaintRootSize + 0.5f;
    uv = saturate(uv);

    return gPaintMask.SampleLevel(gsamLinearClamp, uv, 0).r;
}
...
VertexOut displaceVS(VertexIn vin)
{
    ...
    float2 uv = mul(float4(vin.TexC, 0.0f, 1.0f), gTexTransform).xy;
    float3 posL = vin.PosL;

    float baseHeight = gDisplacementMap.SampleLevel(gsamLinearClamp, uv, 0).r;

    float4 flatPosW = mul(float4(posL, 1.0f), gWorld);
    float sculpt = SamplePaintMask(flatPosW.xz);

    flatPosW.y += baseHeight * gBaseHeightScale + sculpt * gPaintHeightScale;

    vout.PosW = flatPosW;
    vout.PosH = mul(flatPosW, gViewProj);
    vout.TexC = uv;
             ...
             const OutputPatch<VertexIn, 3> tri)
{
    ...
    float4 flatPosW = mul(float4(p, 1.0f), gWorld);
    float sculpt = SamplePaintMask(flatPosW.xz);

    float disp = gDisplacementMap.SampleLevel(gsamAnisotropicClamp, t, 0).r;
    p.y += disp * gBaseHeightScale + sculpt * gPaintHeightScale;
