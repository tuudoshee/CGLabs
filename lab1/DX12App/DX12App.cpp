...
private:
	...
	bool  mSculpting = false;
	float mBrushRadius = 24.0f;
	float mBrushStrength = 0.02f;
	float mBrushStrengthDown = -0.02f;
};
...
void DX12App::OnMouseDown(WPARAM btnState, int x, int y)
{
	...
	if (btnState & MK_RBUTTON)
	{
		...
		{
			mSculpting = true;
			mPendingStrokes.push_back({ x, y, mBrushRadius, mBrushStrength });
		}
		else if (GetAsyncKeyState(VK_CONTROL) & 0x8000)
		{
			mSculpting = true;
			mPendingStrokes.push_back({ x, y, mBrushRadius, mBrushStrengthDown });
		}
	}
...
void DX12App::OnMouseMove(WPARAM btnState, int x, int y)
{
	...
	{
		int dx = abs(x - mLastMousePos.x);
		int dy = abs(y - mLastMousePos.y);
...
void DX12App::BuildPaintMask()
{
	...
	mCommandList->ClearUnorderedAccessViewFloat(uavGpu, uavCpu, mPaintMask.Get(), clear, 0, nullptr);
...
void DX12App::ExecutePaintStrokes()
{
	...
	for (auto& s : mPendingStrokes)
	{
		...
		paintCB.RadiusPx = s.radius;     // по факту это world radius
		...
		mCommandList->SetComputeRootConstantBufferView(
			0,
			mPaintParamsCB->Resource()->GetGPUVirtualAddress());
		...
		// t0 = GBuffer RT1, где лежит depth
		mCommandList->SetComputeRootDescriptorTable(
			2,
			CD3DX12_GPU_DESCRIPTOR_HANDLE(
				mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart(),
